...
public class StaxDefaultSplitter<T> extends AbstractItemCountingItemStreamItemReader<T> 
	implements ResourceAwareItemReaderItemStream<T>, InitializingBean {

	private static Logger logger	=	LoggerFactory.getLogger(StaxDefaultSplitter.class);
	
	private Resource resource;	
	
	private String elementName;
	private Boolean supportDTD	=	Boolean.TRUE;
	...
	public StaxDefaultSplitter() {
		setName(ClassUtils.getShortName(StaxDefaultSplitter.class));
	}
	...
	private void init() throws IOException, XMLStreamException {
		...
		if (!getSupportDTD()) {
			xmlInputFactory.setProperty("javax.xml.stream.supportDTD", getSupportDTD().booleanValue());
		}
	...
	public void readElement(StartElement startElement) throws IOException, XMLStreamException {
		logger.trace("readElement:" + startElement.getName().getLocalPart());
